...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public DBHelper(Context context) {
        //What is this SQLiteDatabase.CursorFactory exactly is when is best scenario to use it
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(MovieContract.FavouriteMovie.SQL_CREATE);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL(MovieContract.FavouriteMovie.SQL_DROP);
        onCreate(db);
    }
    ...
    public long insertInTable(String tableName, String columnHack, ContentValues cv) {
        ...
        try {
            ...
            if (rowid < 0) {
                throw new ValueNotInsertedException("Value Not Inserted in table"+tableName);
            }
